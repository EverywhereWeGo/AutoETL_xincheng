...
def init_ods(excel_schema_name, excel_tab_name):
    ...
    template_str = read_template_file(
        r"C:\Users\Administrator\Desktop\autoload\00_config\scripttemplate\init_ods")
...
def init_ods_with_partiton(excel_schema_name, excel_tab_name):
    ...
    template_str = read_template_file(
        r"C:\Users\Administrator\Desktop\autoload\00_config\scripttemplate\init_ods_with_partitions")
    output_str = template_str.replace("{ODS}", excel_schema_name). \
        replace("{ods_tablename}", excel_tab_name). \
        replace("{SRC}", "SRC"). \
        replace("{src_tablename}", excel_tab_name). \
    ...
    for i in range(0, len(crt_tab_list_arr)):
        ...
        if (match):
            if (crt_tab_list_arr[i][2] == "N"):
                all_str = init_ods(crt_tab_list_arr[i][0], crt_tab_list_arr[i][1])
            elif(crt_tab_list_arr[i][2] == "Y"):
